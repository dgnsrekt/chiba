//! Update-availability check and `chiba update` subcommand.
//!
//! Two pieces:
//!
//! 1. [`run`] — handler for `chiba update`. Detects how chiba was installed
...
//!    a cache under `$XDG_CACHE_HOME/chiba/latest_version.json`. If the cache
...
/// How chiba appears to have been installed, judged by the path of the
...
/// Run the `chiba update` subcommand: detect install method and print
...
pub fn run() -> io::Result<()> {
    ...
    match kind {
        InstallKind::Homebrew => {
            ...
            println!();
            println!("    brew update && brew upgrade dgnsrekt/tap/chiba");
        }
        InstallKind::Cargo => {
            println!("Looks like a `cargo install` build. Update with:");
            println!();
            println!("    cargo install --git https://github.com/dgnsrekt/chiba --force");
        }
        InstallKind::Binary => {
            println!("Looks like a downloaded binary. Grab the latest from:");
            println!();
            println!("    https://github.com/dgnsrekt/chiba/releases/latest");
            println!();
        ...
        InstallKind::Unknown => {
            ...
            println!();
            println!("    brew upgrade dgnsrekt/tap/chiba");
            println!("    cargo install --git https://github.com/dgnsrekt/chiba --force");
            println!("    https://github.com/dgnsrekt/chiba/releases/latest");
        }
    }
...
pub fn detect_kind(exe: &Path) -> InstallKind {
    ...
    // A bare /usr/local/bin/chiba could be either a Homebrew shim (older
...
const RELEASE_URL: &str = "https://api.github.com/repos/dgnsrekt/chiba/releases/latest";
...
fn fetch_latest_body() -> Option<String> {
    let out = Command::new("curl")
        .args([
            ...
            "-A",
            concat!("chiba/", env!("CARGO_PKG_VERSION")),
            RELEASE_URL,
        ])
        .output()
        .ok()?;
...
fn cache_path() -> Option<PathBuf> {
    ...
    Some(base.join("chiba").join("latest_version.json"))
}
...
mod tests {
    ...
    fn detect_kind_homebrew_paths() {
        assert_eq!(
            detect_kind(&PathBuf::from("/opt/homebrew/bin/chiba")),
            InstallKind::Homebrew
        );
        assert_eq!(
            detect_kind(&PathBuf::from(
                "/opt/homebrew/Cellar/chiba/2026.5.3/bin/chiba"
            )),
            InstallKind::Homebrew
        );
        assert_eq!(
            detect_kind(&PathBuf::from("/usr/local/Cellar/chiba/2026.5.3/bin/chiba")),
            InstallKind::Homebrew
        );
        assert_eq!(
            detect_kind(&PathBuf::from(
                "/home/linuxbrew/.linuxbrew/Cellar/chiba/2026.5.3/bin/chiba"
            )),
            InstallKind::Homebrew
        );
    }
    ...
    fn detect_kind_cargo_path() {
        assert_eq!(
            detect_kind(&PathBuf::from("/home/m/.cargo/bin/chiba")),
            InstallKind::Cargo
        );
    }
    ...
    fn detect_kind_falls_back_to_binary() {
        assert_eq!(
            detect_kind(&PathBuf::from("/usr/local/bin/chiba")),
            InstallKind::Binary
        );
        assert_eq!(
            detect_kind(&PathBuf::from("/tmp/chiba")),
            InstallKind::Binary
        );
    }
