...
//! way. This module is the single place that reads the environment
//! (`CHIBA_FILE`/`CHIBA_DIR`, falling back to todo.txt-cli's `TODO_FILE`/
//! `TODO_DIR`, plus `DONE_FILE`) — the core stays env-free.
...
/// Resolve the todo file path. Resolution order:
///
/// * `Some(path)` — an explicit positional CLI argument (TUI only) wins,
///   creating an empty file if it doesn't exist.
/// * `$CHIBA_FILE`, then `$TODO_FILE` — used verbatim if set.
/// * `$CHIBA_DIR/todo.md`, then `$TODO_DIR/todo.md`.
/// * `./todo.md` — if it exists in the current directory.
/// * Otherwise — the bundled sample in the temp dir.
///
/// The `TODO_*` fallbacks are deliberate: an existing todo.txt-cli setup keeps
/// pointing chiba at the same directory without a second config.
///
...
fn env_first(names: &[&str]) -> Option<std::ffi::OsString> {
...
/// [`resolve_path`]'s precedence (arg → `$CHIBA_FILE`/`$TODO_FILE` →
/// `$CHIBA_DIR`/`$TODO_DIR` → `./todo.md`), but when none of those apply it
/// returns [`Target::FirstRun`] so the caller can prompt instead of silently
...
pub fn resolve_target(arg: Option<String>) -> io::Result<Target> {
    let decision = decide_target(
        arg,
        env_first(&["CHIBA_FILE", "TODO_FILE"]),
        env_first(&["CHIBA_DIR", "TODO_DIR"]),
...
pub fn sample_path() -> io::Result<PathBuf> {
    ...
    let pb = dir.join("chiba-sample.md");
